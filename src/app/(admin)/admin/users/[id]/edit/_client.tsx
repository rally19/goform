...
import { adminUpdateUser, adminUpdateUserPassword, adminSignOutUser, adminDeleteForm, adminDeleteAsset, type AdminUser } from "@/lib/actions/admin";
...
export function UserEditClient({ user }: { user: AdminUser }) {
  ...
  return (
    <div className="space-y-6">
                      ...
                      <TableCell>
                        <Badge
                          ...
                          className={cn(
                            ...
                            f.status === "active" && "bg-green-500/5 text-green-600 border-green-500/20",
                            f.status === "draft" && "bg-muted text-muted-foreground border-border",
                            f.status === "closed" && "bg-amber-500/5 text-amber-600 border-amber-500/20"
                          )}
                        >
                          {f.status}
                        </Badge>
      ...
      {/* ── Dialogs (outside Tabs, but inside the outer div) ────────────── */}
